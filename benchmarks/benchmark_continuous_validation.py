...
def run(method='Complete'):
    if method.lower() not in ["complete", "basic", "vmax", "memory", "sorting"]:
        print("You should pass one of the following as the chosen method: Complete, Basic, Sorting, Vmax, Memory")
        return False
    try:
        import rospy
    ...
    if n_random_handles is None:
        ...
        with open(srdf_cylinder, 'w') as output:
            generate_srdf(bvh_file, n_random_handles, output)
    ...
    loadServerPlugin (context, "manipulation-corba.so")
    newProblem()
    client = CorbaClient(context=context)
    client.basic._tools.deleteAllServants()
    client.manipulation.problem.selectProblem (context)
    ...
    print("Using path validation method: Graph-Dichotomy"+method.lower().title())
    ps.selectPathValidation("Graph-Dichotomy"+method.lower().title(), 0)
    ...
    class ClusterComputation:
        ...
        def freeBaseConstraint(self, hi):
            ...
            cnode = self._cgraph.get(graph.nodes[ni])
            c = cnode.configConstraint().getConfigProjector().copy()
        ...
        def fixedBaseConstraint(self, hi):
            ...
            cedge = self._cgraph.get(graph.edges[ei])
            return cedge.targetConstraint().getConfigProjector()
        ...
        def solveTSP(self, armPlanner, cluster, qhome = None, pb_kwargs = {}):
            ...
            for hi, qphi, qhi in cluster:
                ...
                cedge = self._cgraph.get(graph.edges[eir])
    ...
    class InStatePlanner:
        ...
        def setEdge(self, edge):
            ...
            self.cproblem.setSteeringMethod(edgeLoopFree.getSteeringMethod())
            self.cproblem.filterCollisionPairs()

        def setReedsAndSheppSteeringMethod(self):
            sm = ps.hppcorba.problem.createSteeringMethod("ReedsShepp", self.cproblem)
        ...
        def solveTSP(self, configs, resetRoadmapEachTime, pb_kwargs={}):
            ...
            l = len(configs)
            paths, distances = self.computeMatrix(configs,resetRoadmapEachTime, pb_kwargs={})
            if l > 15:
            ...
            for i,j in zip(permutation,permutation[1:]):
                p = paths[i][j] if i < j else paths[j][i].reverse()
                solution.append(p)
    ...
    def add_paths(my_paths):
        for p in my_paths:
            ps.client.basic.problem.addPath()
    
    paths_order = []
    clusters_path
    base_paths
    for i in range(len(base_order)-1):
        index_from = base_order[i]
        index_to = base_order[i+1]
        print("Path from cluster " + str(index_from) +" to cluster " + str(index_to))
        paths_order.append(base_paths[i])
        if index_to is not 0:
            print("Append, i =" + str(i))
            paths_order.append(clusters_path[i])
    add_paths(paths_order)
