...
BOOST_AUTO_TEST_CASE (continuous_validation_straight_TRO)
{
  ...
  PathValidationPtr_t dichotomyBasic (hpp::core::continuousValidation::basic::Dichotomy::create (robot, 0));
...
#pragma omp critical
    ...
    if (configValidation->validate (q1, collisionReport)) {
      ...
      bool res2 (dichotomyBasic->validate  (path, false, validPart, report2));
...
#pragma omp critical
      if (!res1) {
        ...
        BOOST_CHECK (!res3);
        if (res2) {
          hppDout (error, "basic::Dichotomy failed to detect collision for q1="
...
#pragma omp critical
      if (res1) {
        ...
        BOOST_CHECK (res3);
        if (!res2) {
          hppDout (info, "basic::Dichotomy found a collision where discretized did "
    ...
    if (configValidation->validate (q2, collisionReport)) {
      ...
      bool res2 (dichotomyBasic->validate  (path, true, validPart, report2));
