...
#TODO:
#- remove P72, keep table

def shootValidConfig(N):
    for i in range(Nmax):
...
def shootValidConfigArmNeutral(N):
    for i in range(Nmax):
        q = robot.shootRandomConfig()
...
def launch_benchmark(Ntests = 1):
    print("Launching benchmark for", Ntests, "paths")
    for i in range(Ntests):
        # Generate 2 random configurations
        res, q1 = generate_valid_config()
        if res:
            # Generate straight path between the 2 configs
            #TODO turn then go ? of just straight path ?
            # Start the timer
            start = time.time()
            # Test straight path
            # End timer
